...
module GeneratePdf
  ...
  def self.sell user, customer, value_input, value_out, currency_input_id, currency_out_id, created_at, updated_at 
    ...
    Prawn::Document.new(PDF_OPTIONS) do |pdf|
      ...
      pdf.text "Relatório da Venda", :size => 26, :style => :bold, :align => :center
      ...
      pdf.text "Cambio realizando em : <b>#{updated_at.strftime("%d/%m/%y as %H:%M:%S")}</b>", :size => 10, :inline_format => true, :valign => :bottom, :align => :left
