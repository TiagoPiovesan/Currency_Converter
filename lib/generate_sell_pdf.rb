...
module GeneratePdf
  ...
  def self.sell user, customer, value_input, value_out, currency_input_id, currency_out_id, created_at, updated_at
    ...
    Prawn::Document.new(PDF_OPTIONS) do |pdf|
      ...
      pdf.text "Relatório de Venda", :size => 26, :style => :bold, :align => :center
      ...
      Adm: #{user} <br>
      Cliente: #{customer} <br>
      moeda entrada: #{Currency.find(currency_input_id).name} <br>
      moeda saida: #{Currency.find(currency_out_id).name} <br>
      valor entrada: #{value_input} <br>
      valor saida: #{value_out} <br>
      ...
      pdf.text "Troca realizada em : <b>#{created_at.strftime("%d/%m/%y as %H:%M")}</b>", :size => 10, :inline_format => true, :valign => :bottom, :align => :left
      ...
      # Gera no nosso PDF e coloca na pasta public com o nome sell.pdf
      pdf.render_file("public/pdf/venda_#{(DateTime.now).strftime('%d-%m-%y_%H-%M-%S')}.pdf")
    end
  end
end
